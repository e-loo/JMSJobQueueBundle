...
class RunCommand extends Command
{
    ...
    private StyleInterface|null $style = null;
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($this->verbose) {
            $this->style->info('Cleaning up stale jobs');
        }
    ...
    private function runJobs($workerName, int $startTime, int $maxRuntime, int $idleTime, int $maxJobs, array $restrictedQueues, array $queueOptionsDefaults, array $queueOptions): void
    {
        $hasPcntl = extension_loaded('pcntl');

        if ($this->verbose) {
            $this->style->info('Running jobs');
        }

        if ($hasPcntl) {
            $this->setupSignalHandlers();
            if ($this->verbose) {
                $this->style->success('Signal Handlers have been installed.');
            }
        } elseif ($this->verbose) {
            $this->style->warning('PCNTL extension is not available. Signals cannot be processed.');
        }
        ...
        if ($this->verbose) {
            $this->style->info('Entering shutdown sequence, waiting for running jobs to terminate...');
        }
        ...
        if ($this->verbose) {
            $this->style->success('All jobs finished, exiting.');
        }
    }
    ...
    private function setupSignalHandlers(): void
    {
        pcntl_signal(SIGTERM, function (): void {
            if ($this->verbose) {
                $this->style->info('Received SIGTERM signal.');
            }
    ...
    private function startJobs($workerName, int $idleTime, int $maxJobs, array $restrictedQueues, array $queueOptionsDefaults, array $queueOptions): void
    {
        $excludedIds = [];
    ...
    private function getMaxConcurrentJobs(int|string $queue, array $queueOptionsDefaults, array $queueOptions, int $maxConcurrentJobs): int
    {
        ...
        foreach ($this->runningJobs as $i => &$data) {
            ...
            if ('' !== $newOutput && '0' !== $newOutput) {
                $event = $this->eventDispatcher->dispatch(
                    new NewOutputEvent($data['job'], $newOutput, NewOutputEvent::TYPE_STDOUT),
                    'jms_job_queue.new_job_output'
                );
            ...
            if ('' !== $newErrorOutput && '0' !== $newErrorOutput) {
                $event = $this->eventDispatcher->dispatch(
                    new NewOutputEvent($data['job'], $newErrorOutput, NewOutputEvent::TYPE_STDERR),
                    'jms_job_queue.new_job_output'
                );
            ...
            if ($this->verbose) {
                if (!empty($newOutput)) {
                    $this->style->info('Job '.$data['job']->getId().': '.str_replace("\n", "\nJob ".$data['job']->getId().': ', $newOutput));
                }
            ...
            if ($data['job']->getMaxRuntime() > 0 && $runtime > $data['job']->getMaxRuntime()) {
                ...
                $this->style->error($data['job'].' terminated; maximum runtime exceeded.');
            ...
            $this->style->info($data['job'].' finished with exit code '.$data['process']->getExitCode().'.');
    ...
    {
        $stateChangeEvent = $this->eventDispatcher->dispatch(
            new StateChangeEvent($job, Job::STATE_RUNNING),
            'jms_job_queue.job_state_change'
        );

        $newState = $stateChangeEvent->getNewState();
        ...
        $this->runningJobs[] = ['process' => $process, 'job' => $job, 'start_time' => time(), 'output_pointer' => 0, 'error_output_pointer' => 0];
    }
    ...
    private function cleanUpStaleJobs($workerName): void
    {
        ...
        foreach ($staleJobs as $staleJob) {
            ...
            if (!$staleJob->isRetryJob() && count($staleJob->getRetryJobs()) > 0) {
                continue;
            }
            ...
            if (0 !== $proc->run()) {
                ...
                $this->style->info(sprintf('There was an error when marking %s as incomplete: %s', $staleJob, $ex->getMessage()));
            }
        }
    }
