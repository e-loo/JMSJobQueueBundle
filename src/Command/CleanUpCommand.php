...
class CleanUpCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            $this->style->error("Failed to cleanup JMS jobs. An error occurred in {$ex->getFile()} on line {$ex->getLine()}: {$ex->getMessage()}");
    ...
    private function cleanUpExpiredJobs(InputInterface $input): void
    {
        ...
        $incomingDepsSql = $connection->getDatabasePlatform()->modifyLimitQuery('SELECT 1 FROM jms_job_dependencies WHERE dest_job_id = :id', 1);
