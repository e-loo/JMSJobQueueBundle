...
class JobTest extends TestCase
{
    ...
    public function testIsRetryJob(Job $job): void
    {
        $this->assertFalse($job->isRetryJob());
        $this->assertTrue($job->getRetryJobs()->get(0)->isRetryJob());
    }

    #[Depends('testAddRetryJob')]
    public function testGetOriginalJob(Job $job): void
    {
        $this->assertSame($job, $job->getOriginalJob());
        $this->assertSame($job, $job->getRetryJobs()->get(0)->getOriginalJob());
    }
    ...
    private function setField(Job $job, string $field, int $value): void
    {
        $reflectionProperty = new ReflectionProperty($job, $field);
